...
class CommonIntegralsVisualizer:
    ...
    def display_integral_table(self) -> None:
        ...
        for name, formula in formulas.items():
            ...
            print(f"  Example:       ∫({formula['example']}) dx = {formula['antiderivative']} + C")
    ...
    def visualize_integral_as_area(self, func_str: str, interval: Tuple[float, float] = (0, 2),
        ...
        # Plot the function
        plt.plot(data['x_plot'], data['y_plot'], 'b-', linewidth=2, label=f'f(x) = {func_str}')

        # Fill the area under the curve
        plt.fill_between(data['x_fill'], 0, data['y_fill'], alpha=0.3, color='lightblue',
    ...
    def compare_integration_methods(self, func_str: str, interval: Tuple[float, float] = (0, 2)) -> None:
        ...
        # Plot the function
        ax1.plot(data['x_vals'], data['y_vals'], 'b-', linewidth=2, label=f'f(x) = {func_str}')

        # Fill area
        a, b = interval
        ax1.fill_between(data['x_vals'], 0, data['y_vals'], alpha=0.3, color='lightblue')
        ...
        if data['exact_value'] is not None:
            ax2.axhline(y=data['exact_value'], color='red', linestyle='--',
    ...
    def integration_by_parts_demo(self, u_str: str = "x", dv_str: str = "exp(x)") -> None:
        ...
        print(f"\nStep 4: Final result")
        print(f"  ∫ ({data['original']}) dx = {data['uv']} - ({data['remaining_integral']}) + C")
        ...
        try:
            ...
            ax2.fill_between(x_vals[400:600], 0, y_orig[400:600], alpha=0.3, color='lightgreen')
            ...
            # Show the process
            ax4.text(0.1, 0.8, f"∫ {data['original']} dx", fontsize=12, transform=ax4.transAxes)
            ax4.text(0.1, 0.6, f"= {data['uv']} - ∫ {data['v_du']} dx", fontsize=12, transform=ax4.transAxes)
            ax4.text(0.1, 0.4, f"= {data['uv']} - {data['remaining_integral']}", fontsize=12, transform=ax4.transAxes)
            ax4.text(0.1, 0.2, f"= {data['result']} + C", fontsize=12, transform=ax4.transAxes,
    ...
    def substitution_method_demo(self, func_str: str = "2*x*exp(x**2)",
        ...
        try:
            ...
            u_substitution_func = sp.lambdify(sp.Symbol('x'), data['u_substitution'], 'numpy')
            ...
            ax1.fill_between(x_vals[400:600], 0, y_orig[400:600], alpha=0.3, color='lightblue')
            ...
            # Show the process
            ax4.text(0.1, 0.9, f"∫ {data['func_str']} dx", fontsize=12, transform=ax4.transAxes)
            ax4.text(0.1, 0.7, f"Let {substitution}", fontsize=12, transform=ax4.transAxes)
            ax4.text(0.1, 0.5, f"du = {data['du_dx']} dx", fontsize=12, transform=ax4.transAxes)
            ax4.text(0.1, 0.3, f"= ∫ {data['func_in_u']} du", fontsize=12, transform=ax4.transAxes)
            ax4.text(0.1, 0.1, f"= {data['final_result']} + C", fontsize=12, transform=ax4.transAxes,
                    bbox=dict(boxstyle="round,pad=0.3", facecolor="yellow", alpha=0.5))
            ax4.set_xlim(0, 1)
            ax4.set_ylim(0, 1)
            ax4.axis('off')
    ...
    def area_between_curves(self, func1_str: str = "x**2", func2_str: str = "x + 2",
        ...
        # Plot both functions
        plt.plot(data['x_plot'], data['y1_plot'], 'b-', linewidth=2, label=f'f₁(x) = {func1_str}')
        plt.plot(data['x_plot'], data['y2_plot'], 'r-', linewidth=2, label=f'f₂(x) = {func2_str}')

        # Fill area between curves
        plt.fill_between(data['x_fill'], data['y1_fill'], data['y2_fill'],
                        alpha=0.3, color='lightgreen', label='Area between curves')

        # Mark intersection points
        plt.axvline(x=a, color='purple', linestyle='--', alpha=0.7, label=f'x = {a:.2f}')
        plt.axvline(x=b, color='purple', linestyle='--', alpha=0.7, label=f'x = {b:.2f}')
        ...
        print(f"Area = ∫[{a:.3f}, {b:.3f}] |({data['upper_func']}) - ({data['lower_func']})| dx")
        if data['exact_area'] is not None:
    ...
    def volume_of_revolution(self, func_str: str = "sqrt(x)",
        ...
        ax1.plot(data['x_vals'], data['y_vals'], 'b-', linewidth=3, label=f'f(x) = {func_str}')
        ax1.fill_between(data['x_vals'], 0, data['y_vals'], alpha=0.3, color='lightblue')
        ...
        ax2.plot_surface(data['X_surf'], data['Y_surf'], data['Z_surf'], alpha=0.7, cmap='viridis')
        ax2.set_xlabel('x')
        ax2.set_ylabel('y')
        ax2.set_zlabel('z')
        ...
        ax4.text(0.1, 0.8, f'Volume of Revolution', fontsize=14, fontweight='bold', transform=ax4.transAxes)
        ax4.text(0.1, 0.7, f'Function: f(x) = {func_str}', fontsize=12, transform=ax4.transAxes)
        ax4.text(0.1, 0.6, f'Interval: [{interval[0]}, {interval[1]}]', fontsize=12, transform=ax4.transAxes)
        ax4.text(0.1, 0.5, f'Method: {method.capitalize()}', fontsize=12, transform=ax4.transAxes)
        ax4.text(0.1, 0.4, f'Axis: {axis}', fontsize=12, transform=ax4.transAxes)

        if data['exact_volume'] is not None:
            ax4.text(0.1, 0.3, f'Exact Volume: {data["exact_volume"]:.6f}', fontsize=12,
                    transform=ax4.transAxes, bbox=dict(boxstyle="round,pad=0.3", facecolor="yellow", alpha=0.5))

        if data['numerical_volume'] is not None:
            ax4.text(0.1, 0.2, f'Numerical Volume: {data["numerical_volume"]:.6f}', fontsize=12, transform=ax4.transAxes)
            if data['error'] is not None:
                ax4.text(0.1, 0.1, f'Error: ±{data["error"]:.2e}', fontsize=12, transform=ax4.transAxes)
    ...
    while True:
        ...
        while True:
            ...
            try:
                ...
                if sp.simplify(sp.diff(user_expr, sp.Symbol('x')) - sp.diff(correct_expr, sp.Symbol('x'))) == 0:
